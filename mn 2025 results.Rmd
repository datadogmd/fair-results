...
pdf_info_df <- pdf_data(paste(pdfPath, fileList[4], sep = "/"), font_info = TRUE)
...
# Extract low-level data with font information
pdf_info_df <- pdf_data("MN_State_Fair_Results.pdf", font_info = TRUE)

# Combine pages and filter for font names containing "Bold"
# (Note: Check pdf_fonts() first to see exactly how bold fonts are named in your file)
bold_results <- bind_rows(pdf_info_df) %>%
  filter(grepl("Bold|Medi|Black", font_name, ignore.case = TRUE))

# View your bolded entries
head(bold_results$text)
